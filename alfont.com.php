...
class WebDownloader extends Scrapper
{
		...
		foreach($cards as $card) {
			...
			try {
				...
				if ( $post_link ){
					...
					$save_to_ttf_path = $this->get_download_path($post_title, 'ttf');
					$save_to_otf_path = $this->get_download_path($post_title, 'otf');
					$save_to_jpg_path = $this->get_download_path($post_title, 'jpg');
					$save_to_png_path = $this->get_download_path($post_title, 'png');

					// Helper\pre($save_to_jpg_path, $save_to_ttf_path);
					// return;
					
					if ( (file_exists($save_to_ttf_path) || file_exists($save_to_otf_path) ) 
						&& ( file_exists($save_to_jpg_path) || file_exists($save_to_png_path) )
					)  {
						...
						#----- DOWNLOAD THUMBNAIL -----#
						if( ! file_exists($save_to_jpg_path) && ! file_exists($save_to_png_path) ) {
							...
							try {
								...
								// Helper\pre('save_to_jpg_path', $save_to_jpg_path);
								// return;
						...
						if( ! file_exists($save_to_ttf_path) && ! file_exists($save_to_otf_path)) {
							...
							try {
								...
								$direct_font_link = $this->get_element($post_link, 'font-link');
								// Helper\pre('direct_font_link', $direct_font_link); return;
								
								if (! $direct_font_link ){
									echo $this->icon_msg->msg("Couldn't find `font` link!", 'txt-danger', $direct_font_link)->icons('icon-file-pdf', 'icon-unlink');
									echo '</ul>';
									// $i++;
								}
								else {
									
									$save_to_font_path = $this->get_font_save_path( $direct_font_link, $post_title );
									// Helper\pre('save_to_font_path', $save_to_font_path); return;
									
									$this->file_done = $this->download_pdf($direct_font_link, $save_to_font_path);

									if($this->file_done) {
										echo $this->icon_msg->msg("Downloaded", 'txt-success', $direct_font_link)->icons('icon-file-pdf', 'icon-ok');
									}
									else {
										echo $this->icon_msg->msg("Couldn't download!", 'txt-danger', $direct_font_link)->icons('icon-file-pdf', 'icon-cancel');
									}
								}
							}
							catch (Exception $e) {
	...
	#-------------------------------------------------------------------------------------------#
	function get_font_save_path($direct_font_link, $post_title) {
		$font_save_path = '';
		if ($direct_font_link) {
			$list = explode('.', $direct_font_link);
			$type = array_pop($list);
			$font_save_path = $this->get_download_path($post_title, $type);
		}
		if (! $font_save_path) {
			die(" Coudn't find font type");
		}
...
$url = 'https://alfont.com/new-arabic-fonts';
// $folder = 'الديانة الإسلامية';


// $s = new WebDownloader($url, $folder);
$s = new WebDownloader($url);
$s->set_download_file_types('ttf|otf|zip|rar|7z');
$s->set_selector('page-title', 'h1.display-4 strong');
$s->set_selector('posts', '.col-md-12.mb-2');
$s->set_selector('post-title', 'a.card-header>h1', 'find');
// $s->set_selector('post-author', '.book-author>a');
$s->set_selector('post-link', 'a.card-header::attr(href)');
...
$s->set_selector('font-link', '.page-content a.bg-green[href^="https://alfont.com/"]::attr(href)', 'first');
//$s->set_selector('font-link', '.page-content .justify-content-right div:nth-child(3) > a.btn::attr(href)', 'first');
$s->set_selector('next-page', 'a.page-link-btn.page-link-btn-end::attr(href)', 'first', false);
